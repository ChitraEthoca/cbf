...
public class CustomerContactPage_760_763_768 extends BasePage {
	...
	public CustomerContactPage_760_763_768 navigateToWI_AddContact1(Hashtable<String, String> testData)
		...
		try {
			...
			Reporter.log("Step 2 failed" + e.getLocalizedMessage());
		...
		try {
			...
			util.highlighElement(CCLOGNumberEle);
		...
		try {
			...
			String ABACLogEntry = cclogText.getText();
			util.highlighElement(cclogText);
			...
			driver.switchTo().frame("main");
			driver.switchTo().frame("tabPage");
			WebElement comments = driver.findElement(By.xpath("//*[@id=\"DESCRLONG\"]"));
			util.highlighElement(comments);
			driver.switchTo().frame("CC_LOG_GRID");
		...
		try {
			...
			util.highlighElement(customerContactLogWithABACCheckboxele);
			Reporter.log("CC log text is :" + cclogText);
			Thread.sleep(1000);
			Assert.assertTrue(customerContactLogWithABACCheckboxele.isDisplayed());
		...
		try {
			...
			Thread.sleep(1000);
			util.highlighElement(WIsaveSuccessfullConfirmEle);
			Assert.assertTrue(WIsaveSuccessfullConfirmEle.isDisplayed());
